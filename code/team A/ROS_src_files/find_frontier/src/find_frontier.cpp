...
void mapConvert(const nav_msgs::OccupancyGrid::ConstPtr& msg){
  if((IsPoseUpdated==1)&&(pose_ptr_->pose.position.x) > 1.8){
    slam_end++;
    std_msgs::Int32 endmsg;
    endmsg.data=1;
    end_pub.publish(endmsg);
    return;
  }
    ...
    if(distance>0.5)
    {
      if(msg->data[i]==0)
      {
        if(x_ic>2 && x_ic<size_x-3)
        {
          ...
          {
            int nearones_15by15[225];
            int nearones_11by11[121];
            int nearones_9by9[81];
            int nearones_5by5[25];
            int nearones_3by3[9];
            for(int j=0;j<225;j++)
            {
              nearones_15by15[j]=i+size_x*(j/15-7)+(j%15-7);
            }
            for(int j=0;j<121;j++)
            {
              nearones_11by11[j]=i+size_x*(j/11-5)+(j%11-5);
            }
            for(int j=0;j<81;j++)
            {
              nearones_9by9[j]=i+size_x*(j/9-4)+(j%9-4);
            }
            for(int j=0;j<25;j++)
            {
            ...
            if(IsFrontier==1)
            {
              for(int j=0;j<225;j++)
              {
                if(msg->data[nearones_15by15[j]]==100)
                {
                  IsFrontier=0;
                }
              }
...
int main(int argc, char **argv)
{
   ...
   end_pub = nh.advertise<std_msgs::Int32>("TheEnd",1);
   while(ros::ok()&&(slam_end==0)){
     ros::spinOnce();
   }
   ...
   goal_position.pose.orientation.w = 0.1;
   goal_pub.publish(goal_position);
   return 0;
}
