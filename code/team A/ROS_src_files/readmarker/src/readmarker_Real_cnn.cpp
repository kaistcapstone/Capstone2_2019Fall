...
#include "core_msgs/markermsg_R.h"
...
core_msgs::markermsg_R cropped_img_msg;
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  cropped_img_msg.image1_available_R = b_image1;
  cropped_img_msg.image2_available_R = b_image2;

  imencode(".jpg", image_1, cropped_img_msg.cimage1_R.data);
  imencode(".jpg", image_2, cropped_img_msg.cimage2_R.data);
...
{
  Initialization();
  ros::init(argc, argv, "marker_reader_Real");
  ros::NodeHandle nh;
  ros::Rate loop_rate(10);
  input_status_pub = nh.advertise<core_msgs::markermsg_R>("cropped_img_R", 100);
  marker_corner_pub = nh.advertise<core_msgs::markerposition>("marker_pixel_Real_cnn", 100);
  ...
  image_transport::Subscriber sub = it.subscribe("camera/image_Real", 1, imageCallback);
  while(ros::ok()){
    ros::spinOnce();
    // printf("sldfiwejlif");
    loop_rate.sleep();
