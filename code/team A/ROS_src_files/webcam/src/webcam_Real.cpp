...
{
  ros::init(argc, argv, "webcam_Real");   //init ros node. The name of node is decided here.
  ros::NodeHandle nh;  // create node handler
  image_transport::ImageTransport it(nh);  //create image transport and connect it to the node handler created above
  image_transport::Publisher pub = it.advertise("camera/image_Real", 1);  //create a publisher that is connected to image traport(it), the topic name and queue size are decided.
  ...
  cv::VideoCapture cap = cv::VideoCapture(2); //create cv::VideoCapture (*opencv function that capture images from camera)
