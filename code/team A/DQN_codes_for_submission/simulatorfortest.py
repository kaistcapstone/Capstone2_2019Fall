...
max_iter = 400 #220 #180 #300 #130 #99
...
#####################################################################################################################
        ...
        for wall in walls_sampled:
            ...
            #     point_start = -1*map_param["center"]
            #     point_end = 1*map_param["center"]
            ...
            if abs(wall[1]) >= abs(wall[0]) and abs(wall[0]) > 0.2:
...
####################################################################################################
        ...
        if rnum == 1:
            for i in range(26):
                self.obstacles.append(obstacles_temptemp[i+26])
        if rnum == -1:
            for i in range(24):
                self.obstacles.append(obstacles_temptemp[i+28])
#####################################################################################################################
...
##########################################################################################################
        ...
        if self.ball_inscreen_history >= 23:
            reward = reward - 3/130
        if self.wall_approaching_history >= 16:
            reward = reward - 2/130
        #print(self.cont_rot_history)
        if self.ball_inscreen_history != 0 and self.cont_rot_history >= 5:
            reward = reward - 2/130
        if self.cont_rot_history >= 26:
            reward = reward - 4/130
...
##########################################################################################################
            ...
            if cx < high_reward_region_x[-1] + 1 and cx >= high_reward_region_x[0] and ball[0] >= int(ball_blind_ratio*(abs(1.0*ball[1])-ball_blind_bias)):
                if high_reward_region_y[0] <= cy <= high_reward_region_y[-1]:
                    self.balls_in_contact = False
                    if len(self.balls_prev) > 0:
                        if not low_reward_region_x[0] <= int(round(1.0*self.balls_prev[i][0]/trans_scale)) < low_reward_region_x[-1]:
                                    reward = reward - 130/130
                    if len(self.balls_prev) == 0:
                        reward = reward - 130/130
                    if len(self.balls) == 1:
                        reward = reward + 130/130
...
###################################################################################
        ...
        #    self.done = True
        #print(self.cont_rot_history)
        if len(balls_inscreen) != 0 and not action == 0:
            reward = reward - 1/130
        if len(balls_inscreen) == 0 and action == 0:
            reward = reward - 1/130
        ...
        if len(balls_temp) == 0 or self.iter > max_iter or self.ball_inscreen_flag >= 3 * math.ceil(360/rot_scale) + 16:
...
##########################################################################################
        ...
        if action == 1 or action == 2:
            ...
            for obstacle in self.obstacles:
            # if int(abs(1.0*obstacle[0]/trans_scale)) <= 0 and int(abs(1.0*obstacle[1]/trans_scale)) <= 0:
                if obstacle[0] ** 2 + obstacle[1] ** 2 < 7.0 ** 2:
                    wall_rot = True
            if wall_rot == True:
                reward = reward - 2/130
                #print("wall")
        ...
        if action == -1:
##################################################################
# If there's no ball in the sight of robot and do nothing, -3 points
            if len(balls_inscreen) == 0:
                return  - 3/130
#################################################################
            else:
...
####################################################################################################################
        ...
        #         cy = round(ball[1])
        #         if cx ** 2 + cy **2 < 6 ** 2 and ball[0] >= int(ball_blind_ratio*(abs(1.0*ball[1])-ball_blind_bias)):
        #             enable_move = True
...
####################################################################################################################
        ...
        if enable_move:
            ...
            reward = self.get_reward(-1) - 4/130 + reward #-1
