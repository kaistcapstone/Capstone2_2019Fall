...
from utils.replay_bufferfortest import ReplayBuffer
...
def dqn_learing(
    ...
    Q = torch.load('DuelingDDQN_net1029_test.pt')
    print("DuelingDDQN_net1029_test.pt is loaded for additional training")
    #target_Q = q_func(input_arg, num_actions).type(dtype)
    target_Q = torch.load('DuelingDDQN_net1029_test.pt')
    ...
    for t in count():
        ...
        if (t > learning_starts and
            ...
            """ Implementation of DDQN(Double DQN)
             Structure of code from the comparison of
...
#####################################################################################################################
            loss = (current_Q_values - Variable(target_Q_values.unsqueeze(1))).pow(2).mean()
#####################################################################################################################
            optimizer.zero_grad()
            loss.backward()
        ...
        if t % LOG_EVERY_N_STEPS == 0 and t > learning_starts:
            ...
            if abs((best_mean_episode_reward - mean_episode_reward)/best_mean_episode_reward) <= 0.07:
                torch.save(Q, 'DuelingDDQN_net1029prebest_test.pt')
                torch.save(Q.state_dict(),'DuelingDDQN_net1029dictprebest_test.pt')
                print("Best score model saved")
            sys.stdout.flush()
            torch.save(Q, 'DuelingDDQN_net1029pre_test.pt')
            torch.save(Q.state_dict(),'DuelingDDQN_net1029dictpre_test.pt')
            print("Model saved")
