...
#####################################################################################################################
        ...
        if t % LOG_EVERY_N_STEPS == 0 and t > learning_starts:
            ...
            if abs((best_mean_episode_reward - mean_episode_reward)/best_mean_episode_reward) <= 0.07:
                torch.save(Q, 'DuelingDDQN_net1029best_test.pt')
                torch.save(Q.state_dict(),'DuelingDDQN_net1029dictbest_test.pt')
                print("Best score model saved")
            sys.stdout.flush()
            torch.save(Q, 'DuelingDDQN_net1029_test.pt')
            torch.save(Q.state_dict(),'DuelingDDQN_net1029dict_test.pt')
            print("Model saved")
