...
void mapConvert(const nav_msgs::OccupancyGrid::ConstPtr& msg){
  ...
  if(FrontierList.size()!=0)
  {
    ...
    //std::cout << "goal_x: " <<goal_x<<"     goal_y: "<<goal_y<<"     goal_w: "<<std::cout;
    if (sw==0){
  ...
  if (sw){
    ...
    //   goal_y =0.0;
    //   goal_w =1.0;
    //   cout << "goal 0 : 0 : 0" << endl;
    //   }
    //else {    cout<<"goal 1.4 : 0.8 : 0"<<endl;}
    goal_position.pose.position.x = goal_x;
    goal_position.pose.position.y = goal_y;
    goal_position.pose.orientation.w = goal_w;
    goal_pub.publish(goal_position);
...
void goal_sw_Callback(const core_msgs::goal_sw::ConstPtr&  msg)
{
  map_mutex.lock();
  std::cout << "in subscribe call"<< std::endl;
   if (msg->data ==1){
    sw = 1;
std::cout<<"goal_sw=  "<<sw<<std::endl;}
  map_mutex.unlock();
...
int main(int argc, char **argv)
{
   ...
   ros::NodeHandle nh;
   ros::Subscriber sub3 = nh.subscribe<core_msgs::goal_sw>("goal_sw",1, goal_sw_Callback);
